...
def assert_configuration(cfg: DictConfig) -> None:
    ...
    if manifold_early_stage_logging_path != "auto":
        assert manifold_early_stage_logging_path.endswith(".pt")

    manifold_late_stage_logging_path = cfg.path.manifold_late_stage_logging_path
    if manifold_late_stage_logging_path != "auto":
        assert manifold_late_stage_logging_path.endswith(".pt")

    manifold_early_stage_naive_cf_path = cfg.path.manifold_early_stage_naive_cf_path
    if manifold_early_stage_naive_cf_path != "auto":
        assert manifold_early_stage_naive_cf_path.endswith(".pt")

    manifold_late_stage_naive_cf_path = cfg.path.manifold_late_stage_naive_cf_path
    if manifold_late_stage_naive_cf_path != "auto":
        assert manifold_late_stage_naive_cf_path.endswith(".pt")

    manifold_early_stage_moe_cf_path = cfg.path.manifold_early_stage_moe_cf_path
    if manifold_early_stage_moe_cf_path != "auto":
        assert manifold_early_stage_moe_cf_path.endswith(".pt")

    manifold_early_stage_moe_model_selector_path = (
        cfg.path.manifold_early_stage_moe_model_selector_path
    )
    if manifold_early_stage_moe_model_selector_path != "auto":
        assert manifold_early_stage_moe_model_selector_path.endswith(".pt")

    manifold_early_stage_quantile_cf_path = (
        cfg.path.manifold_early_stage_quantile_cf_path
    )
    if manifold_early_stage_quantile_cf_path != "auto":
        assert manifold_early_stage_quantile_cf_path.endswith(".pt")

    manifold_logging_action_prob_model_path = (
        cfg.path.manifold_logging_action_prob_model_path
    )
    if manifold_logging_action_prob_model_path != "auto":
        assert manifold_logging_action_prob_model_path.endswith(".pt")

    manifold_logging_marginal_model_path = cfg.path.manifold_logging_marginal_model_path
    if manifold_logging_marginal_model_path != "auto":
        assert manifold_logging_marginal_model_path.endswith(".pt")

    manifold_early_stage_online_credit_assigned_pg_path = (
        cfg.path.manifold_early_stage_online_credit_assigned_pg_path
    )
    if manifold_early_stage_online_credit_assigned_pg_path != "auto":
        assert manifold_early_stage_online_credit_assigned_pg_path.endswith(".pt")

    manifold_early_stage_online_vanilla_pg_path = (
        cfg.path.manifold_early_stage_online_vanilla_pg_path
    )
    if manifold_early_stage_online_vanilla_pg_path != "auto":
        assert manifold_early_stage_online_vanilla_pg_path.endswith(".pt")

    manifold_early_stage_is_credit_assigned_pg_path = (
        cfg.path.manifold_early_stage_is_credit_assigned_pg_path
    )
    if manifold_early_stage_is_credit_assigned_pg_path != "auto":
        assert manifold_early_stage_is_credit_assigned_pg_path.endswith(".pt")

    manifold_early_stage_is_vanilla_pg_path = (
        cfg.path.manifold_early_stage_is_vanilla_pg_path
    )
    if manifold_early_stage_is_vanilla_pg_path != "auto":
        assert manifold_early_stage_is_vanilla_pg_path.endswith(".pt")

    manifold_early_stage_kernel_is_credit_assigned_pg_path = (
        cfg.path.manifold_early_stage_kernel_is_credit_assigned_pg_path
    )
    if manifold_early_stage_kernel_is_credit_assigned_pg_path != "auto":
        assert manifold_early_stage_kernel_is_credit_assigned_pg_path.endswith(".pt")
