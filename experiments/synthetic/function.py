...
from early_stage_retrieval.synthetic.learner import (
    ActionProbModel,
    ActionProbRegressor,
...
def save_logs(
    ...
    if key_param is not None:
        ...
        manifold_logging_rootdir = (
            f"{manifold_rootdir}/{experiment_name},param={key_param}"
        )

    else:
        logging_rootdir = Path(f"{rootdir}/{experiment_name}")
        manifold_logging_rootdir = f"{manifold_rootdir}/{experiment_name}"

    logging_rootdir.mkdir(parents=True, exist_ok=True)

    if use_manifold:
        with ManifoldClient.get_client(bucket=bucket) as client:
            if not client.sync_exists(manifold_logging_rootdir):
                client.sync_mkdir(manifold_logging_rootdir, recursive=True)
    ...
    if (
        ...
        if use_manifold:
            manifold_logging_dir = f"{manifold_logging_rootdir}/logging"

            with ManifoldClient.get_client(bucket=bucket) as client:
                if not client.sync_exists(manifold_logging_dir):
                    client.sync_mkdir(manifold_logging_dir, recursive=True)

                if trained_logging_early_stage_policy is not None:
                    manifold_logging_early_stage_policy_path = (
                        f"{manifold_logging_dir}/early_stage_policy.pt"
                    )
                    client.sync_put(
                        manifold_logging_early_stage_policy_path,
                        str(logging_early_stage_policy_path),
                        predicate=ManifoldClient.Predicates.AllowOverwrite,
                    )

                if trained_logging_late_stage_policy is not None:
                    manifold_logging_late_stage_policy_path = (
                        f"{manifold_logging_dir}/late_stage_policy.pt"
                    )
                    client.sync_put(
                        manifold_logging_late_stage_policy_path,
                        str(logging_late_stage_policy_path),
                        predicate=ManifoldClient.Predicates.AllowOverwrite,
                    )

                if logging_training_logs is not None:
                    manifold_logging_training_process_dir = (
                        f"{manifold_logging_dir}/training_process"
                    )
                    if not client.sync_exists(manifold_logging_training_process_dir):
                        client.sync_mkdir(
                            manifold_logging_training_process_dir, recursive=True
                        )

                    for key in logging_training_logs.keys():
                        manifold_logging_training_logs_path = (
                            f"{manifold_logging_training_process_dir}/{key}.pt"
                        )
                        local_logging_training_logs_path = (
                            logging_training_process_dir / f"{key}.pt"
                        )
                        client.sync_put(
                            manifold_logging_training_logs_path,
                            str(local_logging_training_logs_path),
                            predicate=ManifoldClient.Predicates.AllowOverwrite,
                        )
    ...
    if key_param is not None:
        ...
        manifold_online_pg_rootdir = f"{manifold_rootdir}/{experiment_name},param={key_param},logging={'na'},credit_assignment={credit_assignment_type},n_candidate_action_train={n_candidate_action_train},seed={random_seed}"

    else:
        online_pg_rootdir = Path(
            f"{rootdir}/{experiment_name}/{experiment_name},logging={'na'},credit_assignment={credit_assignment_type},seed={random_seed}"
        )
        manifold_online_pg_rootdir = f"{manifold_rootdir}/{experiment_name}/{experiment_name},logging={'na'},credit_assignment={credit_assignment_type},n_candidate_action_train={n_candidate_action_train},seed={random_seed}"
    ...
    if (
        ...
        if use_manifold:
            manifold_online_pg_dir = f"{manifold_online_pg_rootdir}/online"

            with ManifoldClient.get_client(bucket=bucket) as client:
                if not client.sync_exists(manifold_online_pg_dir):
                    client.sync_mkdir(manifold_online_pg_dir, recursive=True)

                if trained_online_pg_early_stage_policy is not None:
                    manifold_online_pg_early_stage_policy_path = (
                        f"{manifold_online_pg_dir}/early_stage_policy.pt"
                    )
                    client.sync_put(
                        manifold_online_pg_early_stage_policy_path,
                        str(online_pg_early_stage_policy_path),
                        predicate=ManifoldClient.Predicates.AllowOverwrite,
                    )

                if online_pg_training_logs is not None:
                    manifold_online_pg_training_process_dir = (
                        f"{manifold_online_pg_dir}/training_process"
                    )
                    if not client.sync_exists(manifold_online_pg_training_process_dir):
                        client.sync_mkdir(
                            manifold_online_pg_training_process_dir, recursive=True
                        )

                    for key in online_pg_training_logs.keys():
                        manifold_online_pg_training_logs_path = (
                            f"{manifold_online_pg_training_process_dir}/{key}.pt"
                        )
                        local_online_pg_training_logs_path = (
                            online_pg_training_process_dir / f"{key}.pt"
                        )
                        client.sync_put(
                            manifold_online_pg_training_logs_path,
                            str(local_online_pg_training_logs_path),
                            predicate=ManifoldClient.Predicates.AllowOverwrite,
                        )
    ...
    if key_param is not None:
        ...
        manifold_reg_rootdir = f"{manifold_rootdir}/{experiment_name},param={key_param},logging={logging_type},seed={random_seed}"

    else:
        reg_rootdir = Path(
            f"{rootdir}/{experiment_name}/{experiment_name},logging={logging_type},seed={random_seed}"
        )
        manifold_reg_rootdir = f"{manifold_rootdir}/{experiment_name}/{experiment_name},logging={logging_type},seed={random_seed}"

    rootdir.mkdir(parents=True, exist_ok=True)

    if use_manifold:
        with ManifoldClient.get_client(bucket=bucket) as client:
            if not client.sync_exists(manifold_reg_rootdir):
                client.sync_mkdir(manifold_reg_rootdir, recursive=True)
    ...
    if (
        ...
        if use_manifold:
            manifold_naive_cf_dir = f"{manifold_reg_rootdir}/naive_cf"

            with ManifoldClient.get_client(bucket=bucket) as client:
                if not client.sync_exists(manifold_naive_cf_dir):
                    client.sync_mkdir(manifold_naive_cf_dir, recursive=True)

                if trained_naive_cf_early_stage_policy is not None:
                    manifold_naive_cf_early_stage_policy_path = (
                        f"{manifold_naive_cf_dir}/early_stage_policy.pt"
                    )
                    client.sync_put(
                        manifold_naive_cf_early_stage_policy_path,
                        str(naive_cf_early_stage_policy_path),
                        predicate=ManifoldClient.Predicates.AllowOverwrite,
                    )

                if trained_naive_cf_late_stage_policy is not None:
                    manifold_naive_cf_late_stage_policy_path = (
                        f"{manifold_naive_cf_dir}/late_stage_policy.pt"
                    )
                    client.sync_put(
                        manifold_naive_cf_late_stage_policy_path,
                        str(naive_cf_late_stage_policy_path),
                        predicate=ManifoldClient.Predicates.AllowOverwrite,
                    )

                if naive_cf_training_logs is not None:
                    manifold_naive_cf_training_process_dir = (
                        f"{manifold_naive_cf_dir}/training_process"
                    )
                    if not client.sync_exists(manifold_naive_cf_training_process_dir):
                        client.sync_mkdir(
                            manifold_naive_cf_training_process_dir, recursive=True
                        )

                    for key in naive_cf_training_logs.keys():
                        manifold_naive_cf_training_logs_path = (
                            f"{manifold_naive_cf_training_process_dir}/{key}.pt"
                        )
                        local_naive_cf_training_logs_path = (
                            naive_cf_training_process_dir / f"{key}.pt"
                        )
                        client.sync_put(
                            manifold_naive_cf_training_logs_path,
                            str(local_naive_cf_training_logs_path),
                            predicate=ManifoldClient.Predicates.AllowOverwrite,
                        )

                if late_stage_cf_training_logs is not None:
                    manifold_late_stage_cf_training_process_dir = (
                        f"{manifold_naive_cf_dir}/late_stage_training_process"
                    )
                    if not client.sync_exists(
                        manifold_late_stage_cf_training_process_dir
                    ):
                        client.sync_mkdir(
                            manifold_late_stage_cf_training_process_dir, recursive=True
                        )

                    for key in late_stage_cf_training_logs.keys():
                        manifold_late_stage_cf_training_logs_path = (
                            f"{manifold_late_stage_cf_training_process_dir}/{key}.pt"
                        )
                        local_late_stage_cf_training_logs_path = (
                            late_stage_cf_training_process_dir / f"{key}.pt"
                        )
                        client.sync_put(
                            manifold_late_stage_cf_training_logs_path,
                            str(local_late_stage_cf_training_logs_path),
                            predicate=ManifoldClient.Predicates.AllowOverwrite,
                        )
    ...
    if (
        ...
        if use_manifold:
            manifold_moe_cf_dir = f"{manifold_reg_rootdir}/moe_cf"

            with ManifoldClient.get_client(bucket=bucket) as client:
                if not client.sync_exists(manifold_moe_cf_dir):
                    client.sync_mkdir(manifold_moe_cf_dir, recursive=True)

                if trained_moe_cf_early_stage_policy is not None:
                    manifold_moe_cf_early_stage_policy_path = (
                        f"{manifold_moe_cf_dir}/early_stage_policy.pt"
                    )
                    client.sync_put(
                        manifold_moe_cf_early_stage_policy_path,
                        str(moe_cf_early_stage_policy_path),
                        predicate=ManifoldClient.Predicates.AllowOverwrite,
                    )

                if trained_moe_cf_model_selector is not None:
                    manifold_moe_cf_model_selector_path = (
                        f"{manifold_moe_cf_dir}/model_selector.pt"
                    )
                    client.sync_put(
                        manifold_moe_cf_model_selector_path,
                        str(moe_cf_model_selector_path),
                        predicate=ManifoldClient.Predicates.AllowOverwrite,
                    )

                if moe_cf_training_logs is not None:
                    manifold_moe_cf_training_process_dir = (
                        f"{manifold_moe_cf_dir}/training_process"
                    )
                    if not client.sync_exists(manifold_moe_cf_training_process_dir):
                        client.sync_mkdir(
                            manifold_moe_cf_training_process_dir, recursive=True
                        )

                    for key in moe_cf_training_logs.keys():
                        manifold_moe_cf_training_logs_path = (
                            f"{manifold_moe_cf_training_process_dir}/{key}.pt"
                        )
                        local_moe_cf_training_logs_path = (
                            moe_cf_training_process_dir / f"{key}.pt"
                        )
                        client.sync_put(
                            manifold_moe_cf_training_logs_path,
                            str(local_moe_cf_training_logs_path),
                            predicate=ManifoldClient.Predicates.AllowOverwrite,
                        )
    ...
    if trained_quantile_cf_model is not None or quantile_cf_training_logs is not None:
        ...
        if use_manifold:
            manifold_quantile_cf_dir = f"{manifold_reg_rootdir}/quantile_cf"

            with ManifoldClient.get_client(bucket=bucket) as client:
                if not client.sync_exists(manifold_quantile_cf_dir):
                    client.sync_mkdir(manifold_quantile_cf_dir, recursive=True)

                if trained_quantile_cf_model is not None:
                    manifold_quantile_cf_model_path = (
                        f"{manifold_quantile_cf_dir}/quantile_cf_model.pt"
                    )
                    client.sync_put(
                        manifold_quantile_cf_model_path,
                        str(quantile_cf_model_path),
                        predicate=ManifoldClient.Predicates.AllowOverwrite,
                    )

                if quantile_cf_training_logs is not None:
                    manifold_quantile_cf_training_process_dir = (
                        f"{manifold_quantile_cf_dir}/training_process"
                    )
                    if not client.sync_exists(
                        manifold_quantile_cf_training_process_dir
                    ):
                        client.sync_mkdir(
                            manifold_quantile_cf_training_process_dir, recursive=True
                        )

                    for key in quantile_cf_training_logs.keys():
                        manifold_quantile_cf_training_logs_path = (
                            f"{manifold_quantile_cf_training_process_dir}/{key}.pt"
                        )
                        local_quantile_cf_training_logs_path = (
                            quantile_cf_training_process_dir / f"{key}.pt"
                        )
                        client.sync_put(
                            manifold_quantile_cf_training_logs_path,
                            str(local_quantile_cf_training_logs_path),
                            predicate=ManifoldClient.Predicates.AllowOverwrite,
                        )
    ...
    if (
        ...
        if use_manifold:
            manifold_logging_action_prob_model_dir = (
                f"{manifold_reg_rootdir}/logging_action_prob_model"
            )

            with ManifoldClient.get_client(bucket=bucket) as client:
                if not client.sync_exists(manifold_logging_action_prob_model_dir):
                    client.sync_mkdir(
                        manifold_logging_action_prob_model_dir, recursive=True
                    )

                if trained_logging_action_prob_model is not None:
                    manifold_logging_action_prob_model_path = f"{manifold_logging_action_prob_model_dir}/logging_action_prob_model.pt"
                    client.sync_put(
                        manifold_logging_action_prob_model_path,
                        str(logging_action_prob_model_path),
                        predicate=ManifoldClient.Predicates.AllowOverwrite,
                    )

                if logging_action_prob_model_training_logs is not None:
                    manifold_logging_action_prob_model_training_process_dir = (
                        f"{manifold_logging_action_prob_model_dir}/training_process"
                    )
                    if not client.sync_exists(
                        manifold_logging_action_prob_model_training_process_dir
                    ):
                    ...
                    for key in logging_action_prob_model_training_logs.keys():
                        manifold_logging_action_prob_model_training_logs_path = f"{manifold_logging_action_prob_model_training_process_dir}/{key}.pt"
                        local_logging_action_prob_model_training_logs_path = (
                            logging_action_prob_model_training_process_dir / f"{key}.pt"
                        )
                        client.sync_put(
                            manifold_logging_action_prob_model_training_logs_path,
                            str(local_logging_action_prob_model_training_logs_path),
                            predicate=ManifoldClient.Predicates.AllowOverwrite,
                        )
    ...
    if (
        ...
        if use_manifold:
            manifold_logging_marginal_model_dir = (
                f"{manifold_reg_rootdir}/logging_marginal_model"
            )

            with ManifoldClient.get_client(bucket=bucket) as client:
                if not client.sync_exists(manifold_logging_marginal_model_dir):
                    client.sync_mkdir(
                        manifold_logging_marginal_model_dir, recursive=True
                    )

                if trained_logging_marginal_model is not None:
                    manifold_logging_marginal_model_path = f"{manifold_logging_marginal_model_dir}/logging_marginal_model.pt"
                    client.sync_put(
                        manifold_logging_marginal_model_path,
                        str(logging_marginal_model_path),
                        predicate=ManifoldClient.Predicates.AllowOverwrite,
                    )

                if logging_marginal_model_training_logs is not None:
                    manifold_logging_marginal_model_training_process_dir = (
                        f"{manifold_logging_marginal_model_dir}/training_process"
                    )
                    if not client.sync_exists(
                        manifold_logging_marginal_model_training_process_dir
                    ):
                    ...
                    for key in logging_marginal_model_training_logs.keys():
                        manifold_logging_marginal_model_training_logs_path = f"{manifold_logging_marginal_model_training_process_dir}/{key}.pt"
                        local_logging_marginal_model_training_logs_path = (
                            logging_marginal_model_training_process_dir / f"{key}.pt"
                        )
                        client.sync_put(
                            manifold_logging_marginal_model_training_logs_path,
                            str(local_logging_marginal_model_training_logs_path),
                            predicate=ManifoldClient.Predicates.AllowOverwrite,
                        )
    ...
    if key_param is not None:
        ...
        manifold_offline_pg_rootdir = f"{manifold_rootdir}/{experiment_name},param={key_param},logging={logging_type},credit_assignment={credit_assignment_type},n_candidate_action_train={n_candidate_action_train},seed={random_seed}"

    else:
        offline_pg_rootdir = Path(
            f"{rootdir}/{experiment_name}/{experiment_name},logging={logging_type},credit_assignment={credit_assignment_type},seed={random_seed}"
        )
        manifold_offline_pg_rootdir = f"{manifold_rootdir}/{experiment_name}/{experiment_name},logging={logging_type},credit_assignment={credit_assignment_type},n_candidate_action_train={n_candidate_action_train},seed={random_seed}"
    ...
    if trained_is_pg_early_stage_policy is not None or is_pg_training_logs is not None:
        ...
        if use_manifold:
            manifold_is_pg_dir = f"{manifold_offline_pg_rootdir}/IS"

            with ManifoldClient.get_client(bucket=bucket) as client:
                if not client.sync_exists(manifold_is_pg_dir):
                    client.sync_mkdir(manifold_is_pg_dir, recursive=True)

                if trained_is_pg_early_stage_policy is not None:
                    manifold_is_pg_early_stage_policy_path = (
                        f"{manifold_is_pg_dir}/early_stage_policy.pt"
                    )
                    client.sync_put(
                        manifold_is_pg_early_stage_policy_path,
                        str(is_pg_early_stage_policy_path),
                        predicate=ManifoldClient.Predicates.AllowOverwrite,
                    )

                if is_pg_training_logs is not None:
                    manifold_is_pg_training_process_dir = (
                        f"{manifold_is_pg_dir}/training_process"
                    )
                    if not client.sync_exists(manifold_is_pg_training_process_dir):
                        client.sync_mkdir(
                            manifold_is_pg_training_process_dir, recursive=True
                        )

                    for key in is_pg_training_logs.keys():
                        manifold_is_pg_training_logs_path = (
                            f"{manifold_is_pg_training_process_dir}/{key}.pt"
                        )
                        local_is_pg_training_logs_path = (
                            is_pg_training_process_dir / f"{key}.pt"
                        )
                        client.sync_put(
                            manifold_is_pg_training_logs_path,
                            str(local_is_pg_training_logs_path),
                            predicate=ManifoldClient.Predicates.AllowOverwrite,
                        )
    ...
    if (
        ...
        if use_manifold:
            manifold_kernel_is_pg_dir = f"{manifold_offline_pg_rootdir}/kernelIS"

            with ManifoldClient.get_client(bucket=bucket) as client:
                if not client.sync_exists(manifold_kernel_is_pg_dir):
                    client.sync_mkdir(manifold_kernel_is_pg_dir, recursive=True)

                if trained_kernel_is_pg_early_stage_policy is not None:
                    manifold_kernel_is_pg_early_stage_policy_path = (
                        f"{manifold_kernel_is_pg_dir}/early_stage_policy.pt"
                    )
                    client.sync_put(
                        manifold_kernel_is_pg_early_stage_policy_path,
                        str(kernel_is_pg_early_stage_policy_path),
                        predicate=ManifoldClient.Predicates.AllowOverwrite,
                    )

                if kernel_is_pg_training_logs is not None:
                    manifold_kernel_is_pg_training_process_dir = (
                        f"{manifold_kernel_is_pg_dir}/training_process"
                    )
                    if not client.sync_exists(
                        manifold_kernel_is_pg_training_process_dir
                    ):
                        client.sync_mkdir(
                            manifold_kernel_is_pg_training_process_dir, recursive=True
                        )

                    for key in kernel_is_pg_training_logs.keys():
                        manifold_kernel_is_pg_training_logs_path = (
                            f"{manifold_kernel_is_pg_training_process_dir}/{key}.pt"
                        )
                        local_kernel_is_pg_training_logs_path = (
                            kernel_is_pg_training_process_dir / f"{key}.pt"
                        )
                        client.sync_put(
                            manifold_kernel_is_pg_training_logs_path,
                            str(local_kernel_is_pg_training_logs_path),
                            predicate=ManifoldClient.Predicates.AllowOverwrite,
                        )
