"""Evaluate CF-based baseline methods on synthetic simulation."""
...
def _process(
    ...
    **kwargs,
    ...
    early_stage_naive_cf_policy = load_naive_cf_policy(
        env=env,
        dim_model_emb=dim_model_emb,
        early_stage_naive_cf_path=early_stage_naive_cf_path,
        device=device,
        random_seed=random_seed,
    )
    early_stage_moe_cf_policy = load_moe_cf_policy(
        env=env,
        dim_model_emb=dim_model_emb,
        n_moe_model=n_moe_model,
        early_stage_moe_cf_path=early_stage_moe_cf_path,
        device=device,
        random_seed=random_seed,
    )
    early_stage_moe_model_selector = load_moe_model_selector(
        env=env,
        n_moe_model=n_moe_model,
        early_stage_moe_model_selector_path=early_stage_moe_model_selector_path,
        device=device,
    )
    early_stage_greedy_policy = load_greedy_algorithm(
        env=env,
        dim_model_emb=dim_model_emb,
        early_stage_quantile_cf_path=early_stage_quantile_cf_path,
        device=device,
        random_seed=random_seed,
    )
    moe_model_assignment = optimize_moe_model_assignment(
        logged_dataset=tmp_logged_dataset,
        model_selector=early_stage_moe_model_selector,
    ...
    early_stage_policies = [
        early_stage_naive_cf_policy,
        early_stage_moe_cf_policy,
        early_stage_greedy_policy,
    ]
    model_assignments = [
        None,
        moe_model_assignment,
        None,
    ]
    ...
    for i, algo in enumerate(algorithms):
        performance[algo] = evaluate_policy(
            ...
            model_assignment=model_assignments[i],
...
def process(
    ...
    if setting in ["default", "n_candidate_action_eval"]:
        for random_seed in range(conf["n_random_seed"]):
            ...
            if not Path(conf_["early_stage_moe_cf_path"]).exists():
                raise ValueError("early_stage_moe_cf_path does not exist.")

            if not Path(conf_["early_stage_moe_model_selector_path"]).exists():
                raise ValueError("early_stage_moe_model_selector_path does not exist.")

            if not Path(conf_["early_stage_quantile_cf_path"]).exists():
                raise ValueError("early_stage_quantile_cf_path does not exist.")
        ...
        for key_param in conf[key_param_name]:
            ...
            for random_seed in range(conf["n_random_seed"]):
                ...
                if not Path(conf_["early_stage_moe_model_selector_path"]).exists():
                    raise ValueError(
                        "early_stage_moe_model_selector_path does not exist."
                    )
