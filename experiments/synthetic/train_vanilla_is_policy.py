...
def process(
    ...
    if setting in ["default", "n_candidate_action_eval"]:
        for random_seed in range(conf["n_random_seed"]):
            ...
            if conf["manifold_logging_action_prob_model_path"] == "auto":
                conf_["manifold_logging_action_prob_model_path"] = (
                    f"{manifold_rootdir_}/logging_action_prob_model/logging_action_prob_model.pt"
                )
            ...
            if use_manifold:
                with ManifoldClient.get_client(bucket=bucket) as client:
                    ...
                    if not client.sync_exists(
                        ...
                        raise ValueError(
                            "manifold_logging_action_prob_model_path does not exist."
                        )
                    else:
                        client.sync_get(
                            conf_["manifold_logging_action_prob_model_path"],
