...
To mitigate this issue, we propose a novel **"credit-assigned" policy gradient (CA-PG)**, which computes gradients with respect to the probability that the target item is chosen in any candidate set, i.e. marginalizing over all candidate sets that contain it. 
...
@inproceedings{kiyohara2026credit,
  ...
  year = {2026},
  url = {https://openreview.net/forum?id=jnguJEJmYP},
}
```
...
```bash
cd ~/early_stage_retrieval/experiments/synthetic
export PYTHONPATH="~/early_stage_retrieval/:$PYTHONPATH"
```
...
```bash
python3 train_online_policy.py logs.rootdir=~/early_stage_retrieval/experiments/synthetic/logs
...
```bash
~/early_stage_retrieval/experiments/synthetic/logs/online_early_stage/credit={ALL/CA/TOP1}/{detailed_configs}.pt
```
...
```bash
~/early_stage_retrieval/experiments/synthetic/logs/online_early_stage/training_process/credit={ALL/CA/TOP1}/{action_probs/policy_values/train_losses}/{detailed_configs}.pt
```
...
```bash
~/early_stage_retrieval/experiments/synthetic/conf
```
...
```bash
~/early_stage_retrieval/experiments/synthetic/data/kuairec_small_matrix.csv
```

Then, run the code as follows

```bash
python3 train_online_policy_kuairec.py logs.rootdir=~/early_stage_retrieval/experiments/synthetic/logs path.dataset_path=~/early_stage_retrieval/experiments/synthetic/data/kuairec_small_matrix.csv setting.n_output_action=1 setting.n_candidate_action_train=100 setting.n_candidate_action_eval=100 setting.late_stage_optimality=optimal model.credit_assignment_type=TOP1 setting.start_random_seed=0 model.n_moe_model=1
```
...
```bash
~/early_stage_retrieval/experiments/synthetic/logs/online_early_stage/Kuairec,credit={ALL/CA/TOP1}/{detailed_configs}.pt
```

The training processes will be stored in the following location.

```bash
~/early_stage_retrieval/experiments/synthetic/logs/online_early_stage/training_process/KuaiRec,credit={ALL/CA/TOP1}/{action_probs/policy_values/train_losses}/{detailed_configs}.pt
```
