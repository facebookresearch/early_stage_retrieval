...
class ImportanceSamplingLearner(BasePolicyLearner):
    ...
    def __post_init__(self):
        ...
        if torch.cuda.is_available():
            ...
            self.action_prob_model = self.action_prob_model.to(self.device)
    ...
    def _policy_gradient(
        ...
        rejection_sampling_weight: torch.Tensor,
        ...
        rejection_sampling_weight torch.Tensor, shape (n_samples, n_output_action) or (n_samples, )
            The probability of the action given candidate (non-differential).

        logging_prob: torch.Tensor, shape (n_samples, n_output_action) or (n_samples, )
        ...
        """
        iw = rejection_sampling_weight / logging_prob

        if credit_assignment_type in ["full", "partial"]:
            if credit_assignment_type == "full":
                policy_gradient = -iw * early_stage_log_prob * reward
            else:
                policy_gradient = -iw * early_stage_log_prob.unsqueeze(1) * reward
            ...
            policy_gradient = -iw * early_stage_log_prob * agg_reward.squeeze()
    ...
    def train_early_stage_policy_offline(  # pyre-ignore
        ...
        logging_action_prob_model: nn.Module,
        position_weight: Optional[torch.Tensor] = None,
        credit_assignment_type: str = "full",
        n_candidate_action_train: int = 10,
        val_ratio: float = 0.1,
        ...
        logging_action_prob_model: nn.Module
            The pre-trained model to estimate the action probability.
        ...
        n_candidate_action_train: int, default=10
        ...
        if credit_assignment_type == "none":
            raise ValueError(
                "Currently, only ranking-wise credit-assigned IS is supported. User credit_assignment_type='full' or 'partial' instead."
            )
        ...
        logging_action_prob_model = logging_action_prob_model.to(self.device)
        ...
        with tqdm(range(n_epoch)) as pbar:
            for i, ch in enumerate(pbar):
                ...
                for j in range(n_steps_per_epoch):
                    ...
                    with torch.no_grad():
                        logging_prob_ = logging_action_prob_model(
                            context=context_,
                        ...
                        augmented_actions_ = self.target_late_stage_policy.sample(
                        ...
                        rejection_sampling_weight_ = (
                            action_ids_ == augmented_actions_
                        ).float()
                    ...
                    loss_ = self._policy_gradient(
                        ...
                        rejection_sampling_weight=rejection_sampling_weight_,
...
class KernelImportanceSamplingLearner(BasePolicyLearner):
    ...
    env: Optional[BaseDataGenerator]
        The data generation environment. Either env or action_set must be provided.

    action_set: Optional[torch.Tensor]
        The action set. Either env or action_set must be provided.
    ...
    is_model_free_eval_late_stage_policy: bool = False
    env: Optional[BaseDataGenerator] = None
    action_set: Optional[torch.Tensor] = None
    early_stage_optimizer: Optimizer = Adagrad  # pyre-ignore
    early_stage_optimizer_kwargs: Optional[Dict[str, Any]] = None
    device: torch.device = torch.device("cpu")
    random_seed: Optional[int] = None
    ...
    def __post_init__(self):
        ...
        if self.env is None and self.action_set is None:
            raise ValueError("Either env or action_set must be provided.")
        elif self.action_set is None:
    ...
    def _policy_gradient(
        ...
        kernel_weight: torch.Tensor, shape (n_samples, n_output_action) or (n_samples, )
            The kernel similarity weight between the logging action and the augmented one (non-differential).

        logging_marginal_prob: torch.Tensor, shape (n_samples, n_output_action) or (n_samples, )
        ...
        position_weight: Optional[torch.Tensor]
            The position weight for the reward.

        credit_assignment_type: str, default="full"
            The credit assignment type. Either "full", "partial", or "none".
    ...
    def train_early_stage_policy_offline(  # pyre-ignore
        ...
        n_candidate_action_train: int, default=20
            The number of candidate actions to sample.

        position_weight: Optional[torch.Tensor], (n_output_action, )
            The position weight for the reward.
        ...
        n_candidate_per_model_eval: List[int], default=None
        ...
        logging_marginal_model = logging_marginal_model.to(self.device)
            ...
            for i, ch in enumerate(pbar):
                ...
                for j in range(n_steps_per_epoch):
                    ...
                    with torch.no_grad():
                        ...
                        action_ = self.action_set.retrieve_embeddings(  # pyre-ignore
                            actions=action_ids_
                        )
